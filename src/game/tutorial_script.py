...
스텝/텍스트/앵커/정답판정 로직은 브라우저 쪽(web/static/js/tutorial.js)이
맡는다. 여기(서버)는 "이 챕터를 이런 보드 상태로 시작해줘"만 담당한다 --
고정 덱, 스크립트형 AI, 미리 배치된 스택/손패. 실제 챕터 흐름(어떤
순서로 뭘 가르치는지)은 tutorial.js의 TUTORIAL_STEPS를 봐야 전체
그림이 보인다.
...
`Script.chapters`에 해당하는 이 데이터는 2챕터까지만 다룬다.
"""
...
# 선택/예아니오/재배치 등)는 RandomAI에 위임하는 방어적 설계.
# =============================================================================
class Ch1ScriptedAI:
